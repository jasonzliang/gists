export const meta = {
  ...
  description: 'Deep research (LITE/cheaper) — fewer angles + single-vote verification. Fan-out web searches, fetch sources, verify claims, synthesize a cited report at roughly a third of deep-research cost.',
  whenToUse: 'When the user wants a quick multi-source research report and cost/latency matters more than exhaustiveness. Cheaper, faster variant of deep-research (3 angles, 1 verification vote, fewer sources). For high-stakes research where correctness is critical, use deep-research instead. BEFORE invoking, if the question is underspecified, ask 1-2 clarifying questions, then pass the refined question as args.',
  phases: [{"title":"Scope","detail":"Decompose question (from args) into 3 search angles"},{"title":"Search","detail":"3 parallel WebSearch agents, one per angle"},{"title":"Fetch","detail":"URL-dedup, fetch top 8 sources, extract falsifiable claims"},{"title":"Verify","detail":"1-vote verification per claim (1 refute kills)"},{"title":"Synthesize","detail":"Merge semantic dupes, rank by confidence, cite sources"}],
}

// deep-research-lite: Scope → pipeline(Search → URL-dedup → Fetch+Extract) → 1-vote Verify → Synthesize
// Lite variant of deep-research: fewer angles/sources and single-vote verification for ~1/3 the cost.
// Trade-off: 1 skeptic vote (default-refute) is more aggressive at killing claims → lower recall than deep-research.
// Question is passed via Workflow({name: 'deep-research-lite', args: '<question>'}).
...
const SCOPE_SCHEMA = {
  ...
  properties: {
    ...
    summary: { type: "string" },
    angles: { type: "array", minItems: 3, maxItems: 3, items: {
...
if (!QUESTION) {
  return { error: "No research question provided. Pass it as args: Workflow({name: 'deep-research-lite', args: '<question>'})." }
}
const scope = await agent(
  "Decompose this research question into complementary search angles.\n\n" +
  "## Question\n" + QUESTION + "\n\n" +
  "## Task\n" +
  "Generate 3 distinct web search queries that together cover the question from different angles. Pick the 3 angles that best suit the question's domain. Examples:\n" +
