...
// @downloadURL https://update.sleazyfork.org/scripts/483574/Manga%20Loader%20NSFW%20%2B%20Download.user.js
// @updateURL https://update.sleazyfork.org/scripts/483574/Manga%20Loader%20NSFW%20%2B%20Download.meta.js
// ==/UserScript==
...
//modified begin 1/2
// download image from his url
function downloadImage(url, filename, zip) {
...
// download all images of a chapter and add it to a tachiomy compatible zip
async function downloadChapterImages(zipname, chaptername, images, progressCallback) {
    var zip = new JSZip();
    var folder = zip.folder(chaptername);

    for (var i = 0; i < images.length; i++) {
      var imageUrl = images[i];
      var filename = 'image' + (i + 1) + '.jpg';
      await downloadImage(imageUrl, filename, folder);

      var percentComplete = ((i + 1) / images.length) * 100;
  ...
  function addDownloadButton(chapterUrl) {
    ...
    btnDownload.textContent = 'Download chapter';
    btnDownload.style.position = 'fixed';
    btnDownload.style.bottom = '50px';
    btnDownload.style.right = '20px';
    btnDownload.style.zIndex = '9999';

    var progressBarContainer = document.createElement('div');
    progressBarContainer.style.position = 'fixed';
    progressBarContainer.style.top = '0';
    progressBarContainer.style.width = '100%';
    progressBarContainer.style.zIndex = '9998';
    ...
    btnDownload.addEventListener('click', function () {
        ...
        if (images.length > 0) {
            ...
            var progressCallback = function (percentComplete) {
                ...
                log('Progress: ' + percentComplete.toFixed(2) + '%');
            };
            downloadChapterImages(zipname, chapterNumber, images, progressCallback);
        } else {
